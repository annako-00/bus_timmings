...
def bus(request):
     ab=Stopname.objects.all()
     return render(request,'searchresults.html',{'ab':ab})

def bus_detail(request, brname):
    bus = Stopname.objects.get(pk=brname)
    return render(request, 'busdetail.html', {'bus': bus})